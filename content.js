...
function draftPlayerNames(){const out=[];for(const el of document.querySelectorAll("body *")){if(!visible(el))continue;const n=clean(el.textContent);if(n.length<4||n.length>35||!/^[A-Za-zÀ-ÿ.' -]+$/.test(n))continue;let row=el;for(let i=0;i<6&&row;i++,row=row.parentElement){const t=clean(row.innerText);if(/\b(QB|RB|WR|TE)\d?\b/.test(t)&&(/\bvs\b/i.test(t)||/\s@\s/.test(t))&&(/ADP/i.test(t)||/Proj/i.test(t)||row.getBoundingClientRect().width>300)){out.push({el,name:n});break}}}const seen=new Set();return out.filter(x=>{const k=x.name+"|"+Math.round(x.el.getBoundingClientRect().top);if(seen.has(k))return false;seen.add(k);return true})}
async function exposureForDraft(){
 const data=await safe(()=>chrome.storage.local.get({drafts:[],activeDraftContestById:{},lastSelectedContest:""}));if(!data)return null;const drafts=data.drafts||[];
 const id=(location.pathname.match(/\/draft\/([^/?]+)/)||[])[1];let contest=data.activeDraftContestById?.[id]||data.lastSelectedContest;
