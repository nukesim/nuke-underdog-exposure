...
function knownFullNames(){const m=new Map(),add=n=>{n=String(n||"").trim();if(tokens(n).length>=2)m.set(norm(n),n)};Object.values(playerUniverse||{}).forEach(x=>add(x?.name));Object.values(officialExposure||{}).forEach(x=>add(x?.name));allDrafts.forEach(d=>{const trusted=!syntheticDraft(d);(d.players||[]).forEach(x=>{if(trusted||x?.id)add(x?.name)})});return[...m.values()]}
function scopedOfficial(ds){const s=$("sport").value||"ALL",c=$("contest").value||"ALL",m=new Map(),denom=expectedTotal()||ds.length;for(const o of Object.values(officialExposure||{})){if(!o||!Number.isFinite(Number(o.count))||Number(o.total||0)!==Number(denom||0))continue;if(o.sport&&s!=="ALL"&&String(o.sport).toUpperCase()!==String(s).toUpperCase())continue;if(o.contest&&c!=="ALL"&&contestNorm(o.contest)!==contestNorm(c))continue;const k=norm(o.name);const prev=m.get(k);if(!prev||Number(o.capturedAt||0)>=Number(prev.capturedAt||0))m.set(k,o)}return[...m.values()]}
...
function fullName(raw){
 ...
 const pool=knownFullNames(),exact=pool.find(full=>norm(full)===key);if(exact)return exact;
 if(playerUniverse[key]?.name&&tokens(playerUniverse[key].name).length>=2)return playerUniverse[key].name;
 const candidates=pool.filter(full=>tokens(full).length>tokens(raw).length&&nameMatchStrength(raw,full)>0);
...
function playerStats(ds){
 ...
 const denom=expectedTotal()||ds.length;return [...m.values()].map(x=>({...x,pct:denom?100*x.count/denom:0,denom})).sort((a,b)=>b.pct-a.pct||a.name.localeCompare(b.name))
}
...
function render(){const ds=filtered(),ps=playerStats(ds),cs=comboStats(ds);$("draftCount").textContent=ds.length;$("playerCount").textContent=ps.length;$("comboCount").textContent=cs.length;const q=$("search").value.toLowerCase();if(activeTab==="drafts"){$("view").innerHTML=ds.length?ds.map(d=>`<div class="row"><div><b>${esc(d.contest)}</b><br><small>${esc((d.players||[]).map(p=>fullName(p.name)).join(", "))}</small></div><div></div><div class="num">${new Date(d.capturedAt).toLocaleDateString()}</div></div>`).join(""):empty();return}const rows=(activeTab==="players"?ps:cs).filter(x=>x.name.toLowerCase().includes(q));$("view").innerHTML=rows.length?rows.map(x=>`<div class="row"><div><b>${esc(x.name)}</b><div class="bar"><i style="width:${Math.min(100,x.pct)}%"></i></div></div><div class="num">${x.count}/${activeTab==="players"?(x.denom||expectedTotal()||ds.length):ds.length}</div><div class="pct">${x.pct.toFixed(1)}%</div></div>`).join(""):empty()}
