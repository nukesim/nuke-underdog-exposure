...
function fullName(raw){
 ...
 if(playerUniverse[key])return playerUniverse[key].name||String(raw);
 const candidates=Object.entries(playerUniverse).filter(([full])=>aliases(full).includes(key));
 return candidates.length===1?(candidates[0][1].name||candidates[0][0]):String(raw);
}
function playerStats(ds){
 ...
 for(const [key,o] of Object.entries(officialExposure||{})){
  ...
  const name=playerUniverse[key]?.name||m.get(key)?.name||key.replace(/\b\w/g,x=>x.toUpperCase());
...
function comboStats(ds){const m=new Map();ds.forEach(d=>{const n=[...new Set((d.players||[]).map(p=>p.name).filter(Boolean))].sort();for(let i=0;i<n.length;i++)for(let j=i+1;j<n.length;j++){const k=n[i]+" + "+n[j];m.set(k,(m.get(k)||0)+1)}});return [...m].map(([name,count])=>({name,count,pct:ds.length?100*count/ds.length:0})).sort((a,b)=>b.pct-a.pct||a.name.localeCompare(b.name))}
...
async function load(){const d=await chrome.storage.local.get({drafts:[],exposureScope:{sport:"ALL",contest:"ALL"},lastSelectedContest:"ALL",playerUniverse:{},officialExposure:{}});allDrafts=d.drafts||[];playerUniverse=d.playerUniverse||{};officialExposure=d.officialExposure||{};const scope=d.exposureScope||{sport:"ALL",contest:d.lastSelectedContest||"ALL"};buildFilters(scope.sport,scope.contest);render();status(allDrafts.length?`Ready · ${allDrafts.length} drafts tracked`:"No drafts captured yet","ok");try{const [tab]=await chrome.tabs.query({active:true,currentWindow:true});if(tab?.id)await chrome.tabs.sendMessage(tab.id,{type:"NUKE_FORCE_SCAN"})}catch{} }
$("sport").addEventListener("change",async()=>{buildFilters($("sport").value,"ALL");await persist();render()});$("contest").addEventListener("change",async()=>{await persist();render()});$("search").addEventListener("input",render);$("refresh").addEventListener("click",load);document.querySelectorAll(".tab").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));b.classList.add("active");activeTab=b.dataset.tab;render()}));chrome.storage.onChanged.addListener(async(ch,a)=>{if(a!=="local")return;if(ch.drafts||ch.officialExposure||ch.playerUniverse){const d=await chrome.storage.local.get({drafts:[],exposureScope:{sport:"ALL",contest:"ALL"},officialExposure:{},playerUniverse:{}});allDrafts=d.drafts||[];officialExposure=d.officialExposure||{};playerUniverse=d.playerUniverse||{};buildFilters(d.exposureScope.sport,d.exposureScope.contest);render();status(`Ready · ${allDrafts.length} drafts tracked`,"ok")}});load();